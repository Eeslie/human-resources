...
export default function Recruitment() {
  ...
  const [interviews, setInterviews] = useState([]);
	...
	// Job Card Component with Salary Edit
	function JobCardWithSalary({ job, allApplicants, onUpdate }) {
		const [isEditingSalary, setIsEditingSalary] = useState(false);
		const [salaryValue, setSalaryValue] = useState(job.salary_range || job.salary || '');
		
		const handleSaveSalary = async () => {
			const res = await fetch(`/api/recruitment/${job.id}`, {
				method: 'PUT',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ id: job.id, salary_range: salaryValue })
			});
			if (res.ok) {
		...
		const handleCancelEdit = () => {
			setIsEditingSalary(false);
			setSalaryValue(job.salary_range || job.salary || '');
		};
						...
						{isEditingSalary ? (
							<div className="flex items-center gap-2">
								<input
									...
									className="border border-slate-300 rounded px-2 py-1 text-xs w-40 text-black"
							...
							<div className="flex items-center gap-2">
								<span className="text-slate-700">
									{job.salary_range || job.salary || 'Not set'}
								</span>
								<button
	...
	function JobCreateModal({ open, onClose, onSubmit }) {
		...
		const handleSubmit = async (e) => {
			...
			setFormData({ job_title: '', department_id: '', location: '', type: 'Full-time', salary: '', status: 'Active' });
		};
  ...
  return (
                    ...
                    <div className="space-y-3">
                      {jobs.length === 0 ? (
                        <div className="bg-slate-50 border border-slate-200 rounded-lg p-6 text-center">
                          <p className="text-slate-500 text-sm">No job postings yet. Create your first job posting to get started.</p>
                        </div>
                      ) : (
                      ...
                      {jobs.length === 0 ? (
                        <tr>
                          <td colSpan="7" className="px-6 py-8 text-center text-slate-500">
                            No job postings yet. Click "Create Job Posting" to add your first job.
                          </td>
                        </tr>
                      ) : (
                        jobs.map((job) => (
            ...
            {activeTab === 'applicants' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {Array.isArray(allApplicants) && allApplicants.length > 0 ? (
                    allApplicants.map((applicant) => (
                    <div key={applicant.id} className="bg-white border border-slate-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                      <div className="flex items-center space-x-3 mb-4">
                        <div className="w-12 h-12 bg-gradient-to-r from-purple-500 to-purple-600 rounded-full flex items-center justify-center text-white font-semibold text-lg">
                          {applicant.avatar || (applicant.full_name || applicant.name || '?').charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <h4 className="font-semibold text-slate-800">{applicant.full_name || applicant.name}</h4>
                          <p className="text-sm text-slate-600">{applicant.position || 'No position'}</p>
                        </div>
                      </div>
                    ...
                    <div className="col-span-full bg-slate-50 border border-slate-200 rounded-lg p-8 text-center">
                      <p className="text-slate-500 text-sm mb-3">No applicants yet. Add your first applicant to get started.</p>
                      <button 
                        onClick={() => setApplicantAddOpen(true)} 
    ...
    <JobCreateModal 
      ...
      onSubmit={async (formData) => {
        ...
        const body = { 
          vacancy_id, 
          vacancy_date: new Date().toISOString().slice(0,10), 
          ...formData,
          salary_range: formData.salary || formData.salary_range
        };
